...
# Startskript für GPU Mining (Equihash 192)
cat <<EOF > ~/start_gpu_mining.sh
#!/bin/bash
cd ~/SRBMiner-Multi
./SRBMiner-MULTI --algorithm equihash192_7 --gpu --pool eu-de01.miningrigrentals.com:3333 --wallet watis23.352395 --password x
EOF
...
curl -H "Content-Type: application/json" -X POST -d '{"content": "✅ Vast.ai Mining Setup abgeschlossen. GPU (Equihash192_7) & CPU (RandomX) Miner wurden gestartet."}' $DISCORD_WEBHOOK

# Hinweis für den Benutzer
echo "✅ GPU-Mining (Equihash192_7) läuft in Screen 'mining_gpu'"
